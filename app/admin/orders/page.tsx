...
export default async function OrdersPage() {
  ...
  return (
    <main className="p-5 lg:p-8">
      ...
      <p className="mt-2 text-slate-500">Manage fulfilment and order state.</p>
              ...
              <tr key={order.id} className="border-b">
                ...
                <td className="p-4">
                  <form action={updateOrder} className="flex gap-2">
                    ...
                    <select name="status" defaultValue={order.status} className="rounded-lg border px-2 py-1.5">
                      {statuses.map((status) => <option key={status} value={status}>{status}</option>)}
                    </select>
                    <button className="rounded-lg bg-slate-900 px-3 text-xs text-white">Save</button>
                  </form>
                </td>
                <td className="p-4 text-xs text-slate-500 font-mono">
                  {order.shipping_address ? <pre>{JSON.stringify(order.shipping_address, null, 2)}</pre> : "—"}
                </td>
              </tr>
            ))}
          </tbody>
        </table>
        {!data?.length && <p className="p-12 text-center text-slate-500">No orders.</p>}
      </div>
    </main>
  )
}
